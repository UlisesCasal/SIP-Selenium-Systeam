...
function printSummary(chrome, firefox) {
  ...
  console.log(`Chrome  — ${chrome.executionMs}ms — ${chrome.products.length} productos`);
  console.log(`Firefox — ${firefox.executionMs}ms — ${firefox.products.length} productos`);
  console.log(`Títulos en común: ${common.length}/${Math.max(chromeTitles.length, firefoxTitles.length)}`);
  console.log('─'.repeat(60));
