...
describe('retry', () => {
  it('dispara 3 veces ante TimeoutException y luego falla', async () => {
    ...
    await expect(retry(mockFn, { retries: 3, delayMs: 10, logger: { warn: jest.fn() } }))
  ...
  it('tiene éxito en el segundo intento tras un TimeoutException', async () => {
    const mockFn = jest.fn()
      .mockImplementationOnce(() => { throw new error.TimeoutException('timeout'); })
      .mockImplementationOnce(() => 'success');

    const result = await retry(mockFn, { retries: 3, delayMs: 10, logger: { warn: jest.fn() } });
  ...
  it('falla inmediatamente si no hay reintentos', async () => {
    ...
    await expect(retry(mockFn, { retries: 0, delayMs: 10, logger: { warn: jest.fn() } }))
