...
const mockDriver = {
  sendAndGetDevToolsCommand: jest.fn().mockResolvedValue(undefined),
};
const mockBuilder = {
...
describe('BrowserFactory', () => {
  ...
  it('crea Chrome con flags stealth y headless', async () => {
    ...
    expect(mockDriver.sendAndGetDevToolsCommand).toHaveBeenCalledWith(
      'Page.addScriptToEvaluateOnNewDocument',
      expect.objectContaining({ source: expect.stringContaining('navigator') }),
    );
  });

  it('crea Firefox con preferencias y tamaño de ventana', async () => {
    await expect(BrowserFactory.create('firefox', false)).resolves.toBe(mockDriver);

    expect(mockFirefoxOptions.setPreference).toHaveBeenCalledWith('dom.webdriver.enabled', false);
    expect(mockFirefoxOptions.setPreference).toHaveBeenCalledWith('useAutomationExtension', false);
