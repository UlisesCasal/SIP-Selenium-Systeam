...
class SearchResultsPage {
  ...
  async getProducts(limit = 10) {
    ...
    logger.info(`[SearchResultsPage] Extrayendo ${max}/${items.length} resultados`);

    for (let index = 0; index < max; index++) {
      try {
        const item = items[index];
        const rawText = await item.getText();
        const title = await this._textFromSelectors(item, TITLE_SELECTORS, false);
        const priceText = await this._textFromSelectors(item, PRICE_SELECTORS, false);
        const link = await this._extractLink(item);
        const officialStoreText = await this._textFromSelectors(item, OFFICIAL_STORE_SELECTORS, true);
        ...
        logger.warn(`[SearchResultsPage] Producto ${index + 1} omitido: ${error.message}`);
      }
    }
  ...
  async _findItems() {
    const locators = this.itemLocator ? [this.itemLocator, ...ITEM_LOCATORS] : ITEM_LOCATORS;
  ...
  async _textFromSelectors(element, selectors, optional) {
    ...
    throw new Error(`Texto requerido no encontrado con selectores: ${selectors.join(', ')}`);
  }

  async _extractLink(element) {
    for (const selector of ['a.poly-component__title', 'a.ui-search-link', 'a[href*="/"]', 'a']) {
      try {
        const href = await element.findElement(By.css(selector)).getAttribute('href');
