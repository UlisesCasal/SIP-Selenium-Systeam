...
class MercadoLibreScraper {
  ...
  async _fetchFromApi(product, limit = 10) {
    ...
    if (!response.ok) {
      throw new Error(`Fallback API devolvió ${response.status} para "${product}"`);
    }
    ...
    const products = items.slice(0, limit).map((item) => {
      ...
      const cuotas = (inst && inst.rate === 0 && inst.quantity > 1)
    ...
    logger.info(`[MercadoLibreScraper] Fallback API ok para "${product}": ${products.length} productos`);
    return products;
  }
