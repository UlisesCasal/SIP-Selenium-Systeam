...
const makeBrowserFactory = () => ({
  create: jest.fn().mockResolvedValue({ quit: jest.fn().mockResolvedValue(undefined) }),
});
...
describe('MercadoLibreScraper', () => {
  describe('_fetchFromApi', () => {
    ...
    it('lanza error cuando la API devuelve 403', async () => {
      ...
      await expect(scraper._fetchFromApi('bicicleta rodado 29', 10))
    ...
    it('filtra items sin título o precio', async () => {
      global.fetch = jest.fn().mockResolvedValue({
        ...
        json: jest.fn().mockResolvedValue({
          results: [
            { title: '', price: 100, permalink: 'https://a.com', shipping: {}, installments: null },
            { title: 'Válido', price: 0, permalink: 'https://b.com', shipping: {}, installments: null },
            { title: 'Válido', price: 100, permalink: 'https://c.com', shipping: { free_shipping: false }, installments: null },
          ],
        }),
      });
    ...
    it('lanza error cuando la API no devuelve resultados', async () => {
      ...
      await expect(scraper._fetchFromApi('test', 10))
        .rejects.toThrow('Fallback API sin resultados');
    });
  });
  ...
  describe('run() — cadena de fallback', () => {
    it('usa datos estáticos cuando Selenium y API fallan', async () => {
      ...
      const scraper = new MercadoLibreScraper(makeConfig({ maxRetries: 0 }), { browserFactory, writer });

      // Selenium falla
      jest.spyOn(scraper, 'scrapeProduct').mockRejectedValue(new Error('bot detected'));
      // API falla
      global.fetch = jest.fn().mockResolvedValue({ ok: false, status: 403 });
    ...
    it('usa la API cuando Selenium falla y la API responde', async () => {
      ...
      jest.spyOn(scraper, 'scrapeProduct').mockRejectedValue(new Error('timeout'));
      global.fetch = jest.fn().mockResolvedValue({
        ok: true,
        json: jest.fn().mockResolvedValue({ results: [{ title: 'Bici', price: 100000, permalink: 'https://x.com', shipping: { free_shipping: false }, installments: null }] }),
      });
